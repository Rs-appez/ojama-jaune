...
class Tournament:
    ...
    def __init__(self, ctx, duelists):
        ...
        self.participants: dict = {}  # discord.name : challonge_id
        self.ctx = ctx
    ...
    async def create_tournament(self):
        ...
        if response.status_code == 200:
            ...
            if await self.add_members() == 200:
                await self.ctx.send(
                    f"Tournoi crée : {Tournament._challonge_url+self.url} "
                )
            else:
    ...
    def get_participant(self):
        ...
        if response.status_code == 200:
            ...
            for index, participant in enumerate(response.json()):
                self.participants[self.duelists[index].name] = participant[
                    "participant"
                ]["id"]
    ...
    async def move_player(self, matches):
        ...
        for index, m in enumerate(matches.json()):
            player1 = list(mydict.keys())[
                list(mydict.values()).index(m["match"]["player1_id"])
            ]
            player2 = list(mydict.keys())[
                list(mydict.values()).index(m["match"]["player2_id"])
            ]
    ...
    async def set_draw(self, duelist, score):
        ...
        data = {"match": {"winner_id": "tie", "scores_csv": f"{score}-{score}"}}
        ...
        if response.status_code == 200:
            await self.ctx.send(
                "https://tenor.com/view/mark-wahlberg-wahlberg-tie-oscar-%E5%B9%B3%E6%89%8B-gif-9081826"
            )
