...
class Letter_view(View):
    ...
    class Button_letter(Button):
        ...
        async def callback(self, interaction):
            if not self.click:
                ...
                if await self.current_view.hangman.check_letter(self.letter):
    ...
    def __init__(self, hangman, vowel):
        ...
        if vowel:
            hangman.vowel_view = self
        else:
            hangman.consonant_view = self

    def __init_button(self, vowel):
        if vowel:
            letters = self.__letter_vowel
        else:
            letters = self.__letter_consonant
